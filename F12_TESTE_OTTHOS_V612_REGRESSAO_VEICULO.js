(async()=>{
  ...
  try{
    add('Versão V612 carregada',api.version==='V612_RENDER_COMERCIAL_SAFE',api.version);
    ...
    add('Service Worker do escopo atual é V612',regs.length===0||urls.some(u=>u.includes('v=612')),urls.join(' | ')||'Não disponível');
  ...
  console.group(`OTTHOS V612 — TESTE VEÍCULO/ENTRADAS ${score}%`);console.table(results);console.log({score,aprovados:ok,total:results.length,results});console.groupEnd();
  document.getElementById('v612-test-panel')?.remove();
  const panel=document.createElement('div');panel.id='v612-test-panel';panel.style.cssText='position:fixed;inset:12px;z-index:2147483647;background:#071522f5;color:white;border:2px solid #5de1ff;border-radius:18px;padding:16px;overflow:auto;font:14px system-ui';
  panel.innerHTML=`<button style="float:right;padding:10px" onclick="this.parentElement.remove()">Fechar</button><h2>Teste V612 — ${score}%</h2><p>${ok} de ${results.length} aprovados.</p><p><b>Nota:</b> a física é validada por passos fixos, sem depender do requestAnimationFrame, que pode ser reduzido enquanto o foco está no DevTools.</p><table style="width:100%;border-collapse:collapse">${results.map(x=>`<tr style="background:${x.Status==='OK'?'#164c2c':'#692132'}"><td style="padding:8px;border-bottom:1px solid #ffffff22">${x.Teste}</td><td style="padding:8px"><b>${x.Status}</b></td><td style="padding:8px">${x.Detalhes}</td></tr>`).join('')}</table>`;
